...
var CloudTop = (function($, window, document, undefined) {
  
  // Expose innards of CloudTop.
  
  return {
    go: function() {
      for (var i in CloudTop.init) {
        CloudTop.init[i]();
      }
    },
    init: {
      ...
      clock: function() {
        ...
        // Update every 60 seconds -- UPDATED.
		// The reason this was changed to every second is because let's say
		// someone came to the page when the clock was 30 seconds into a minute
		// and they look at the analog clock simultaneously.  Even though there
		// may not be any seconds indicator on the "digital" display, the two
		// will be out of sync if the updating isn't done as such.
		
        setTimeout(CloudTop.init.clock, 1000);
      ...
      desktop: function() {
		...
		// Cancel right-click: LET THIS BE CUSTOMIZABLE.
		...
		*/
        ...
        d.on('dblclick', 'a.icon, .menu li a', function() {
        ...
        d.on('dblclick', 'div.window_top', function() {
          CloudTop.util.window_resize(this);
        });
      ...
      wallpaper: function() {
        ...
        if ($('#desktop').length) {
          $('body').prepend('<img id="wallpaper" class="abs" src="assets/img/wallpaper/current.jpg" />');	// make this configurable, too.
        }
      }
    },
    util: {
